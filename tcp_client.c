...
void execution( int internet_socket )
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        int num1 = rand() % 10;
        int num2 = rand() % 10;
        char op = rand() % 3 == 0 ? '+' : rand() % 3 == 1 ? '-' : '*';
        
        // Send operation to server
        char operation[16];
        sprintf(operation, "%d%c%d", num1, op, num2);
        send(internet_socket, operation, strlen(operation), 0);
        printf("Sent: %s\n", operation);

        // Receive result from server
        char result[16];
        int bytes_received = recv(internet_socket, result, sizeof(result), 0);
        if (bytes_received < 0) {
    ...
    send(internet_socket, "STOP", 4, 0);
    printf("Sent: STOP\n");

    // Receive OK message from server
    char ok[3];
    int bytes_received = recv(internet_socket, ok, sizeof(ok), 0);
    if (bytes_received < 0) {
        perror("Failed to receive OK message from server");
    }
    ok[bytes_received] = '\0';
    printf("Received: %s\n", ok);

    // Send goodbye message to server
    send(internet_socket, "KTNXBYE", 8, 0);
    printf("Sent: KTNXBYE\n");
